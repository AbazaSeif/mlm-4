<li>
	<div id="comment{{$replycomment->id}}">
		...
		<br/>Reply to: <span class="time"><a href="{{ URL::to_action("news@view", array($article->id, $article->slug)) }}#comment{{$comment->id}}">{{ date("M j,Y g:ia", strtotime($comment->created_at)) }}</a></span> by
	...
	<div class="meta">
		<span class="time"><a href="{{ URL::to_action("news@view", array($article->id, $article->slug)) }}#comment{{$replycomment->id}}">{{ date("M j,Y g:ia", strtotime($replycomment->created_at)) }}</a></span>
		<div class="actions">
