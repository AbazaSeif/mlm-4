...
class Teams_Controller extends Base_Controller {
	...
	public function __construct() {
		...
		$this->filter("before", "auth")->only(array("new", "leave", "edit", "edit_meta", "add_owner", "owner_invite", "invite_user", "invite"));
		$this->filter("before", "csrf")->on("post")->only(array("new", "leave", "edit_meta", "add_owner", "owner_invite", "invite_user", "invite"));
	}
	...
	public function post_leave($id) {
		...
		if (Input::has("username")) {
			...
			if(!$user) {
				Messages::add("error", "Can't remove user! User not found");
		...
		if($team->is_owner($user) && $team->users()->where_owner("1")->count() == 1 && $user == Auth::user()) {
			Messages::add("error", "Please assign a new team owner before leaving");
		}
		if($team->is_owner($user) && $user != Auth::user()) { //Don't let owners kick other owners (They can speak to admins to get them removed if need be)
		...
		else
		{
			...
			if ($user != Auth::user()) { Messages::add("success", "User removed from team!"); }
			else { Messages::add("success", "Left Team!"); }
		}
		if ($user != Auth::user()) { return Redirect::to_action("teams@edit", array($team->id)); }
		return Redirect::to_action("teams@view", array($team->id));
	}
	...
	public function get_edit($id) {
		...
		$owners = $team->users()->with("owner")->where('owner', '!=', 'null')->get();
	...
	public function post_add_owner($id) {
		...
		$team->users()->attach($user->id, array("owner" => 0));
